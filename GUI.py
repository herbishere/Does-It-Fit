...
PySimpleGUI.ChangeLookAndFeel('DC4405')      
 # ------ Menu Definition ------ #      
...
window = PySimpleGUI.Window('Will I Sleep this Semester? ', layout, default_element_size=(40, 1), grab_anywhere=False)      
...
while True:
    ...
    if (exists1 != 'None'):
        first_class = ClassStats(file, str(values['class1']))
        class_uno = (first_class.courseName + ': ' + '\nAverage Estimated Time Required: ' + str(int(first_class.timeTotalMean * summer_multiplier)) + '\xB1' + str(format((first_class.timeTotalMean - first_class.timeMinMean)*summer_multiplier, '.1f')) + " hrs" + '\nAverage Expected Difficulty: ' + str(format(first_class.difficultyMean, '.1f')))
        class_total_mean += first_class.timeTotalMean
    if (exists2 != 'None'):
        second_class = ClassStats(file, values['class2'])
        class_dos = (second_class.courseName + ': ' + '\nAverage Estimated Time Required: ' + str(int(second_class.timeTotalMean * summer_multiplier)) + '\xB1' + str(format((second_class.timeTotalMean - second_class.timeMinMean)*summer_multiplier, '.1f')) + " hrs" + '\nAverage Expected Difficulty: ' + str(format(second_class.difficultyMean, '.1f')))
        class_total_mean += second_class.timeTotalMean
    if (exists3 != 'None'):
        third_class = ClassStats(file, values['class3'])
        class_tres = (third_class.courseName + ': ' + '\nAverage Estimated Time Required: ' + str(int(third_class.timeTotalMean * summer_multiplier)) + '\xB1' + str(format((third_class.timeTotalMean - third_class.timeMinMean)*summer_multiplier, '.1f')) + " hrs" + '\nAverage Expected Difficulty: ' + str(format(third_class.difficultyMean, '.1f')))
        class_total_mean += third_class.timeTotalMean
    if (exists4 != 'None'):
        fourth_class = ClassStats(file, values['class4'])
        class_cuatro = (fourth_class.courseName + ': ' + '\nAverage Estimated Time Required: ' + str(int(fourth_class.timeTotalMean * summer_multiplier)) + '\xB1' + str(format((fourth_class.timeTotalMean - fourth_class.timeMinMean)*summer_multiplier, '.1f')) + " hrs" + '\nAverage Expected Difficulty: ' + str(format(fourth_class.difficultyMean, '.1f')))
    ...
    if (time_difference >= 0):
        answer = 'You WILL get sleep this semester!' + ' You went '  + str(time_difference) + ' hours under your free time.'
    else:
        answer = 'You will NOT get sleep this semester!' + '\nYou went ' + str(time_difference * (-1)) + ' hours over your free time.'
    if (exists1 != 'None'):
        class_uno = (first_class.courseName + ': ' + '\nAverage Estimated Time Required: ' + str(int(first_class.timeTotalMean * summer_multiplier)) + '\xB1' + str(format((first_class.timeTotalMean - first_class.timeMinMean)*summer_multiplier, '.1f')) + " hrs" + '\nAverage Expected Difficulty: ' + str(format(first_class.difficultyMean, '.1f')))
